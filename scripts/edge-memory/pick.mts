...
const nm = JSON.parse(fs.readFileSync(`${ROOT}/public/data/edgeNightmare.json`,"utf8"));
...
function parseComm(comm:string){
  ...
  const inner = comm.slice(comm.indexOf("[", br>=0?br:0)+1, comm.lastIndexOf("]"));
  let depth=0, ci=-1;
  for(let i=0;i<inner.length;i++){const c=inner[i]; if(c==="[")depth++; else if(c==="]")depth--; else if(c===","&&depth===0){ci=i;break;}}
  let A=inner, B="";
  if(ci>=0){A=inner.slice(0,ci).trim(); B=inner.slice(ci+1).trim();}
  return {setup, A, B};
}
const moveCount = (s:string)=> s.split(/\s+/).filter(Boolean).length;
...
function pickBest(key:string){
  ...
  const rows = cands.map(alg=>{
    let comm=""; try{const r=commutator.search({algorithm:alg}); comm=r&&r[0]?r[0]:"";}catch{comm="";}
    return {alg, comm, mlen: moveCount(alg)};
  }).filter(r=>/\[.*,.*\]/.test(r.comm));
  ...
  const enriched = rows.map(r=>{
    ...
    const compound = r.comm.includes("+") || /\[.+\[/.test(r.comm);
  ...
  enriched.sort((a,b)=> a.score-b.score || a.mlen-b.mlen || a.setupMoves-b.setupMoves || (Number(b.pure)-Number(a.pure)) || a.comm.length-b.comm.length);
  const e=enriched[0];
  return {key, comm:e.comm, alg:e.alg, mlen:e.mlen, setup:e.setup, A:e.A, B:e.B, setupMoves:e.setupMoves, pure:e.pure, core:e.core, compound:e.compound};
}
...
let done=0;
for(const k of myKeys){ const p=pickBest(k); if(p) picks[k]=p; done++;
  if(done%100===0){ fs.writeFileSync(outFile, JSON.stringify(picks)); process.stderr.write(`[${start}-${end}] ${done}/${myKeys.length}\n`);} }

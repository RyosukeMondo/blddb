...
## Reverse lookup (pick stickers)

The **Pick stickers** toolbar button opens an overlay for going the other direction: instead of choosing a case from the family list, click the two target stickers on an unfolded cube net and jump straight to the case they identify. The net is the same `CubeNet` component and geometry/colors used everywhere else in the trainer, generalized with a `pickable` mode: the 22 valid edge stickers (every two-letter facelet id except the buffer's own UF/FU pair) become focusable buttons, UF/FU render distinctly as the inert buffer, and centers/corners are dimmed and hidden from assistive tech.

The first click sets target 1, the second sets target 2 (both shown as numbered badges on the net and in a live "UF → t1 → t2" readout); clicking a sticker on the same piece as target 1 (e.g. UR after RU) shows an inline message instead of setting target 2, and clicking a set target again clears it. **Swap order** exchanges t1/t2, i.e. jumps to the inverse case; **Clear** resets the picker. Once both targets are set, a preview shows the family name and the exact algorithm that will load — resolved through the same `resolveVariantKey` order as everywhere else, so a remembered "my alg" choice or the "prefer LR mirror" default is respected — followed by an **Open case** button.

A text input accepts the same two ids typed instead of clicked, in any of `RU FR`, `ru-fr`, or `RU→FR` (whitespace, hyphens or an arrow as the separator; case-insensitive). Enter in that field validates the pair and, since a valid pair always resolves to a case, opens it in the same keystroke — the picker's "type and go" path, distinct from the "pick two stickers, then press Open case" path for clicks. Only the app's own facelet ids are recognized — a lettering scheme's per-sticker letters are a different id system and are out of scope here.

The lookup and parsing logic is pure and framework-free: `isValidTargetFacelet`, `isSamePiece`, `validateTargetPair`, `caseIdForTargets` and `parseTargetPairInput` live in `engine.ts` (no case data needed, so `scripts/uf-trainer/verify.mts` can exercise them directly against `catalog.json`), and `lookupCaseByTargets` in `catalog.ts` is a thin catalog-aware wrapper that resolves a validated pair to a case id, familyIndex and caseIndex. `npm run test:uf` checks all 22 × 20 = 440 ordered pairs of targets on distinct pieces resolve to the case whose cycle is exactly `[UF, t1, t2]`, that same-piece pairs and buffer stickers are rejected, and that the typed parser accepts every documented format and rejects garbage input.
